...
module.exports = function (root, app) {
  ...
  var _modules = [];
  var _boot = function _boot(paths) {
    config = _initConfig();
  ...
  var _initThemes = function _initThemes() {
  ...
  var _initAppLogger = function _initAppLogger() {

    winstonKnex = require('./winston/transports/knex');

    var appLogger = new winston.Logger({
      transports: [new winston.transports.Knex({ tableName: 'logs', knexInstance: app.get('knex') })]
    });

    app.set('appLogger', appLogger);
  };

  var _initStaticMiddlewares = function _initStaticMiddlewares() {
    for (var name in _modules) {
      app.use('/' + name, express.static(_modules[name].getPublicPath()));
    }
  };

  var _initSecurityMiddleware = function _initSecurityMiddleware() {
  ...
  var _initBaseMiddlewares = function _initBaseMiddlewares() {
    app.use(logger('dev'));
  ...
  var _initDB = function _initDB() {
    var knex = require('knex')(config.db);
    app.set('knex', knex);
  };

  var _initErrorhandler = function _initErrorhandler() {
  ...
  var _initRoutes = function _initRoutes() {
  ...
  var _initViews = function _initViews() {
    // init view
    var viewPaths = [root + '/views'];
    var nunjucksEnv = new nunjucks.Environment(new nunjucksModuleLoader(_modules, { paths: viewPaths }), {
      autoescape: false
    });
  ...
  var _initModules = function _initModules(paths) {
