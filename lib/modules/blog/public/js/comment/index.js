...
(function ($, drafTerbit) {
    ...
    drafTerbit.comments.dt = $("#comment-data-table").dataTable({
        columns: [{ data: 'id', orderable: false, searchable: false }, { data: 'author_name' }, { data: 'content' }, { data: 'post_id' }]
    });
    ...
    // style all pending
    var stylePendingRow = function stylePendingRow() {
    ...
    $(document).on('click', '.comment-action .status', function (e) {
        ...
        $.post(drafTerbit.deskUrl + 'blog/comment/status', {
            id: $(this).data('id'),
            status: status,
    ...
    $(document).on('click', '.comment-action .trash', function (e) {
        e.preventDefault();
        $.post(drafTerbit.adminUrl + 'blog/comment/quick-trash', {
            id: $(this).data('id'),
            csrf: drafTerbit.csrfToken
        }, function (data) {
    ...
    $(document).on('click', '.comment-action .reply', function (e) {
        ...
        var form = ['<div class="inline-form">', '<textarea style="width:100%" class="form-control">', '</textarea>', '<div style="margin-top:5px;">', '<button type="button" class="btn btn-xs btn-default inline-form-cancel">' + __('Cancel') + '</button>', '<button type="button" class="btn btn-xs btn-primary pull-right inline-form-submit" data-post-id="' + postId + '" data-id="' + id + '">' + __('Submit') + '</button>', '</div>', '</div>'].join('');
    ...
    $(document).on('click', '.inline-form-submit', function () {
        ...
        if (comment.trim() !== '') {
            $.post(drafTerbit.adminUrl + 'blog/comment/quick-reply', {
    ...
    filterByStatus = function filterByStatus(status) {
        ...
        drafTerbit.comments.dt.api().ajax.url(drafTerbit.deskUrl + "blog/comment/data").load(function () {
    ...
    changeUncreateAction = function changeUncreateAction(s) {
        if (s === 'trashed') {
    ...
    //status-filter
    $('.comment-status-filter').on('change', function () {
        var s = $(this).val();
        filterByStatus(s);
        changeUncreateAction(s);
    });

    $('#comment-index-form').ajaxForm(function (response) {
