...
class DashboardWidget extends Base
{
    ...
    {
        $model = $this['extension']->get('system')->model('Log');
        $logs = $model->recent();

        foreach ($logs as &$log) {
            $log = (object)$log;
            $log->time = $this['time']->createFromTimestamp($log->time)->diffForHumans();
            $log->formattedMsg = $this['log.formatter']->format($log->message);
        }
    ...
    {
        $shortcuts = $this['extension']->data('shortcuts');
