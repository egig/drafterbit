...
if (! function_exists('wysiwyg')) {
    ...
    function wysiwyg($name, $default = null, $attr = "")
    {
        ...
        $path = base_url('system/src/Base/Resources/public/assets');
        $src = base_url('system/vendor/web/ckeditor/ckeditor.js');
        $browserUrl = admin_url('files/browser?mode=image');
        $wpmore = base_url('/system/src/Base/Resources/public/assets/ckeditor-custom/plugins/wpmore/plugin.js');
    ...
    function asset_url($path)
    {
        if ('@' === $path[0]) {
            ...
            if ($ext == 'vendor') {
                ...
                $realPath = app('extension')->get($ext)->getResourcesPath('public');
                $base = str_replace(app('path.public'), '', $realPath).'/';
            }
...
if (!function_exists('has_permission')) {
    ...
    {
        return app('extension')->get('user')->model('Auth')->userHasPermission($accessKey);
    }
}